 ...
 */
public class LanguageQualityBenchmark {
...
*        This allows to extract the doc name for search results,
   */
  public LanguageQualityBenchmark(QualityQuery qqs[], QualityQueryParser qqParser, 
      IndexSearcher searcher, String docNameField, float smooth) {
  ...
  public  QualityStats [] execute(Judge judge, SubmissionReport submitRep, 
    ...
    for (int i=0; i<nQueries; i++) {
      ...
      LanguageModelQuery newQ = new LanguageModelQuery(smooth);
      for(Term t : terms) {
              newQ.addTerm(t.createTerm(t.text()));
      }
      // search with this query 
      long t1 = System.currentTimeMillis();
      TopDocs td = searcher.search(newQ,null,maxResults);
      long searchTime = System.currentTimeMillis()-t1;
      //most likely we either submit or judge, but check both 
      if (judge!=null) {
        stats[i] = analyzeQueryResults(qq, newQ, td, judge, qualityLog, searchTime);
      }
      if (submitRep!=null) {
